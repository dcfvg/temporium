...
class button_manuel(Frame):
    ...
    def display_button(self):
        self.Button_B_HIGH = tkinter.Button(self,  text ="BR HAUT", command = self.Button_B_HIGH)        
        self.Button_B_HIGH.pack()
        
        self.Button_B_LOW = tkinter.Button(self,  text ="BR BAS", command = self.Button_B_LOW)        
    ...
    def Button_save(self):
        ...
        if not self._type_detection == "NULL" :
            """file name :  config/config_calibration_BR_LOW.txt"""
            file = open("config/config_calibration_BR_"+self._type_detection+ ".txt", "w")
            
            file.write("Temporium" + "\n")
            file.write("CONFIGURATION_CALIBRATION : BR : "+self._type_detection +  "\n")
