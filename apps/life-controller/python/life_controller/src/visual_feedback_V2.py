...
class visual_feedback_V2(Canvas):
    ...
    def draw(self):
        ...
        for item in self.current_state.occupied_volume : 
            if item == "M1" or item == "M2": 
                self.itemconfig(item, cnf) = self.create_rectangle(self.container_position[item][0]*self.winfo_width(), (self.container_position[item][1]+self.container_dimension[item][1]*(1-self.current_state.get_occupied_volume(item)))*self.winfo_height(),\
                ...
                self.containers_filled[item] = self.create_rectangle(self.container_position[item][0]*self.winfo_width(), (self.container_position[item][1]+self.container_dimension[item][1]*(1-self.current_state.get_occupied_volume(item)))*self.winfo_height(),\
    ...
    def draw_init(self) :
        ...
        for item in self.containers : 
            self.delete(self.containers[item])
        for item in self.containers_filled : 
            self.delete(self.containers_filled[item])
        for item in self.pumps : 
            self.delete(self.pumps[item])
        for item in self.wires : 
            self.delete(self.wires[item])
        ...
        for item in self.container_position : 
            self.containers[item] = self.create_rectangle(self.container_position[item][0]*self.winfo_width(), self.container_position[item][1]*self.winfo_height(),\
        ...
        for item in self.pumps_positions : 
            ...
            self.pumps[item] = self.create_oval((self.pumps_positions[item][0][0]-0.01)*self.winfo_width(),(self.pumps_positions[item][0][1]-0.01)*self.winfo_height(),(self.pumps_positions[item][0][0]+0.01)*self.winfo_width(),(self.pumps_positions[item][0][1]+0.01)*self.winfo_height(), fill="red")
            
            """creation of wires"""
            self.wires[item] = [self.create_line(self.pumps_positions[item][0][0]*self.winfo_width(), self.pumps_positions[item][0][1]*self.winfo_height(), self.pumps_positions[item][1][0]*self.winfo_width(),self.pumps_positions[item][0][1]*self.winfo_height(), fill = "black", dash=(4, 4)),\
        ...
        for item in self.current_state.state_pumps : 
            if self.current_state.get_state_pumps(item) :
                self.pumps[item] = self.create_oval((self.pumps_positions[item][0][0]-0.01)*self.winfo_width(),(self.pumps_positions[item][0][1]-0.01)*self.winfo_height(),(self.pumps_positions[item][0][0]+0.01)*self.winfo_width(),(self.pumps_positions[item][0][1]+0.01)*self.winfo_height(), fill="green")
                """from the container_out to the pump"""
                """from the pump to the container_in"""
                self.wires[item] = [self.create_line(self.pumps_positions[item][0][0]*self.winfo_width(), self.pumps_positions[item][0][1]*self.winfo_height(), self.pumps_positions[item][1][0]*self.winfo_width(),self.pumps_positions[item][0][1]*self.winfo_height(), fill = "green", dash=(4, 4)),\
