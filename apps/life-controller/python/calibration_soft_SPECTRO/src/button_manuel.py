...
class button_manuel(Frame):
    ...
    def display_button(self):
        self.Button_B_HIGH = tkinter.Button(self,  text ="Concentration MAX", command = self.Button_B_HIGH)        
        self.Button_B_HIGH.pack()
        
        self.Button_B_LOW = tkinter.Button(self,  text ="Concentration MIN", command = self.Button_B_LOW)        
        self.Button_B_LOW.pack()
        
        self.Button_B_OPT = tkinter.Button(self,  text ="Concentration OPT", command = self.Button_B_OPT)        
        self.Button_B_OPT.pack()
    ...
    def Button_B_HIGH(self) :
        ...
        for item in self.parent.visual_feedback.dict_rect_crop : 
            ...
            try : 
                value = int(self.parent.spectro_level.get_level(self.parent.file, item,  crop))
                self.parent.visual_feedback.set_spectro_level(item, value)
    ...
    def Button_B_OPT(self) :
        ...
        """set the type of detection in order to be able to save"""
        self._type_detection = "OPT"
    
    def Button_save(self):
        print("SAVING : ")
        if not self._type_detection == "NULL" :
            """file name :  config/config_calibration_SPECTRO_LOW.txt"""
            file = open("config/config_calibration_SPECTRO_"+self._type_detection+ ".txt", "w")
            
            file.write("Temporium" + "\n")
            file.write("CONFIGURATION_CALIBRATION : SPECTRO : "+self._type_detection +  "\n")
